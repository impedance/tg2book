...
Run `python -m venv .venv && source .venv/bin/activate` if you need isolation. Install dependencies with `pip install -r requirements.txt`. Start the bot via `./start.sh` or `python bot.py` once `TELEGRAM_BOT_TOKEN` (and Dropbox secrets) are exported. Use `python dropbox-loader.py <local_path>` to seed Dropbox content. For iteration, tail logs with `tail -f bot.log`.
