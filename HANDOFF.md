...
### Нерешённая проблема
`/reauth` отдаёт `The confirmation code has expired (caused by SignInRequest)` — **мгновенно**, код вводится через секунды после получения.

Три гипотезы опровергнуты практикой. Наиболее вероятная текущая причина: **Telegram флагует аккаунт после множества неудачных попыток** и немедленно возвращает EXPIRED для новых кодов (anti-spam).

## Как починить (рекомендация)
