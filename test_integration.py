...
@pytest.mark.asyncio
async def test_full_pipeline_text_to_dropbox(mock_update, mock_context):
    """
    ...
    mock_update.message.text = "Integration Architecture\n\nThis is a black-box test that verifies things end-to-end without tightly coupling to implementations."
    
    real_dm = _load_real_dropbox_module()
    
    with patch.object(bot, "dropbox_module", real_dm), \
        ...
        res = await converter.handle_message(mock_update, mock_context)
        
        # Verify Telegram replied with success summary
        assert mock_update.message.reply_text.call_count == 2
        last_call_args = mock_update.message.reply_text.call_args_list[-1][0]
...
@pytest.mark.asyncio
async def test_full_pipeline_document_to_dropbox(mock_update, mock_context):
    """
    ...
    file_mock = MagicMock()
    file_mock.download_to_drive = mock_download_to_drive
